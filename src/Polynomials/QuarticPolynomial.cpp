...
QuarticPolynomial::QuarticPolynomial(fixp_s xs, fixp_s_d vxs, fixp_s_dd axs,
        fixp_s_d vxe, fixp_s_dd axe, fixp_t t):
        a0(xs), a1(vxs) {
        a2 = axs>>1;
    // #ifdef USE_RECORDER
...
fixp_s_d QuarticPolynomial::calc_first_derivative(fixp_t t) {
    // return a1 + 2 * a2 * t + 3 * a3 * pow_2<fixp_time_2>(t) + 4 * a4 * pow_3<fixp_time_3>(t);
    // return a1+t*(2*a2+t*(3*a3+4*a4*t));
    fixp_intermediate ans = 3.0*a3+4.0*a4*t;
    ans = 2*a2+t*ans;
    ans = a1+t*ans;
    return ans;
}

fixp_s_dd QuarticPolynomial::calc_second_derivative(fixp_t t) {
    // return 2 * a2 + 6 * a3 * t + 12 * a4 * pow_2<fixp_time_2>(t);
    fixp_intermediate ans = 6.0*a3+12.0*a4*t;
    ans = 2.0*a2+t*ans;
    return ans;
}

fixp_s_ddd QuarticPolynomial::calc_third_derivative(fixp_t t) {
