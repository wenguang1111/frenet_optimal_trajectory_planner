...
float CubicSpline2D::calc_curvature(float t){
    ...
    #ifdef USE_RECORDER
            Recorder::getInstance()->saveData<double>("CubicSpline2D::calc_curvature::dx", dx);
            Recorder::getInstance()->saveData<double>("CubicSpline2D::calc_curvature::ddx", ddx);
            Recorder::getInstance()->saveData<double>("CubicSpline2D::calc_curvature::dy", dy);
            Recorder::getInstance()->saveData<double>("CubicSpline2D::calc_curvature::ddy", ddy);
            Recorder::getInstance()->saveData<double>("CubicSpline2D::calc_curvature::k", k);
    #endif
    return k;
}
...
float CubicSpline2D::calc_yaw(float t) {
    ...
    #ifdef USE_RECORDER
        Recorder::getInstance()->saveData<double>("CubicSpline2D::calc_yaw::dx", dx);
        Recorder::getInstance()->saveData<double>("CubicSpline2D::calc_yaw::dy", dy);
        Recorder::getInstance()->saveData<double>("CubicSpline2D::calc_yaw::yaw", yaw);
    #endif
    return yaw;
}
...
float CubicSpline2D::find_s(float x, float y, float s0) {
    ...
    do {
        ...
        si += 0.1;
        #ifdef USE_RECORDER
            Recorder::getInstance()->saveData<double>("CubicSpline2D::find_s::px", px);
            Recorder::getInstance()->saveData<double>("CubicSpline2D::find_s::py", py);
            Recorder::getInstance()->saveData<double>("CubicSpline2D::find_s::s_closest", s_closest);
            Recorder::getInstance()->saveData<double>("CubicSpline2D::find_s::closest", closest);
            Recorder::getInstance()->saveData<double>("CubicSpline2D::find_s::dist", dist);
        #endif
    } while (si < s.back());
    return s_closest;
}
