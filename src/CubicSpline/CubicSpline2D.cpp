...
void CubicSpline2D::calc_s(const vector<float>& x,
    ...
    for (int i = 0; i < nx - 1; i++) {
        ...
        s.push_back(cum_sum);
        #ifdef USE_RECORDER
            Recorder::getInstance()->saveData<float>("CubicSpline2D::dx", dx.back());
            Recorder::getInstance()->saveData<float>("CubicSpline2D::dy", dy.back());
            Recorder::getInstance()->saveData<float>("CubicSpline2D::s", s.back());
        #endif
    }
...
float CubicSpline2D::calc_yaw(float t) {
    ...
    #ifdef USE_RECORDER
        Recorder::getInstance()->saveData<float>("CubicSpline2D::yaw_dx", dx);
        Recorder::getInstance()->saveData<float>("CubicSpline2D::yaw_dy", dy);
        Recorder::getInstance()->saveData<float>("CubicSpline2D::yaw", yaw);
    #endif
    return yaw;
}
...
CubicSpline2D::remove_collinear_points(vector<float> x, vector<float> y) {
    ...
    for (size_t i = 2; i < x.size()-1; i++) {
        ...
        #ifdef USE_RECORDER
            Recorder::getInstance()->saveData<float>("CubicSpline2D::x_", x_.back());
            Recorder::getInstance()->saveData<float>("CubicSpline2D::y_", y_.back());
        #endif
    }
